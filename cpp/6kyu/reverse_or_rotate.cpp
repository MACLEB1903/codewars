...
public:
    ...
    {
      if (sz == 0) { return ""; }
      string res = strng.substr(0, strng.size() / sz * sz);
  
      for (int i = 0; i < res.size(); i += sz)
      {
        auto begin = next(res.begin(), i);
        auto end   = next(res.begin(), i + sz);
        int sum = accumulate(begin, end, 0, RevRot::acc);
        if (sum % 2) { rotate(begin, begin + 1, end); }
        else { reverse(begin, end); }
      }

...
string zombie_shootout(unsigned zombies, unsigned distance, unsigned ammo){
  distance *= 2;
  ...
  if (remainedRange > 0 || remainedAmmo > 0){
    if (remainedRange < remainedAmmo){
      return "You shot " + to_string(zombies - remainedAmmo) + " zombies before being eaten: ran out of ammo.";
    } else {
      return "You shot " + to_string(zombies - remainedRange) + " zombies before being eaten: overwhelmed.";
    }
  }
  
  return "You shot all " + to_string(zombies) + " zombies.";
}
